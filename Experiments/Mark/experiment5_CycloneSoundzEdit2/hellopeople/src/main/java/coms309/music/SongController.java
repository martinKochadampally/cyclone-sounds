...
public class SongController {
    ...
    @GetMapping("playlists/artist")
    public HashMap<String, List<Song>> createArtistPlaylist() {
        HashMap<String, List<Song>> playlists = new HashMap<>();

        for (Song s : songList.values()) {
            List<Song> list = playlists.get(s.getArtist());
            if (list == null) {
                list = new ArrayList<>();
                playlists.put(s.getArtist(), list);
            }
            list.add(s);
        }
        return playlists;
    }
    @GetMapping("playlists/genre")
    public HashMap<String, List<Song>> createGenrePlaylist(){
        HashMap<String, List<Song>> playlists = new HashMap<>();

        for(Song s : songList.values())
        {
            List<Song> list = playlists.get(s.getGenre());
            if(list == null){
                list = new ArrayList<>();
                playlists.put(s.getGenre(), list);
            }
            list.add(s);
        }
        return playlists;
